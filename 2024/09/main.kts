...
fun List<Int?>.compactFiles(): List<Int?> {
    ...
    tailrec fun compactFile(disk: List<Int?>, fileIndex: Int): List<Int?> {
        ...
        val file = disk
            .dropWhile { it != fileIndex }
            .takeWhile { it == fileIndex }
        val indexOfOpenSpot =
            disk
                .takeWhile { it != fileIndex }
                .windowed(file.size)
                .indexOfFirst { it.all { it == null } }
        when (indexOfOpenSpot) {
            -1 -> return compactFile(disk, nextIndex)

            else -> {
                val pre = disk.take(indexOfOpenSpot)
                val post = disk.drop(indexOfOpenSpot).drop(file.size).map { it.takeIf { it != fileIndex } }
